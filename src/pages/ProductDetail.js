...
<select
  id="size-select"
  value={selectedSize ? JSON.stringify(selectedSize) : JSON.stringify(product.sizes[defaultSize])}
  ...
  {product?.sizes
    ...
    ?.map((size, index) => (
      <option key={index} value={JSON.stringify(size)}>
        {size?.size} ml - {formatPrice(size?.price)} {size.quantity <= 3 ? `(only ${size.quantity} left)` : ''}
      </option>
    ))}
</select>
